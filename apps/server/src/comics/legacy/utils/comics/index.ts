      ...
      list.match(/<a href="(.*?)" title="(.*?)"/gim).forEach((value, index) => {
        value.replace(
          ...
          (...args) => {
            ...
            try {
              fs.mkdirSync(
                path.join(__dirname, cfg.downloadDir, obj.title, args[2])
              );
            } catch (error) {
              // console.log(error);
            }
          ...
          String(v).replace(/p;}\((.*?),\{\}/im, ($0) => {
            ...
            cInfo.files.forEach((value, index) => {
              setTimeout(() => {
                req(
                  encodeURI(
                    `${cInfo.path}${cInfo.files[index]}?cid=${cInfo.cid}&md5=${cInfo.sl.md5}`.replace(
                      /\\/gi,
                      "/"
                    )
                  ),
